...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        .setDescription("<:AuraDown:721175040376438824> | Please input a number, not a **letter** or **word.**")
        .setColor(colours.bot_white)
        if (isNaN(args[0])) return message.channel.send(imputNumbers) 
        let numberLessThen100 = new Discord.MessageEmbed()
        .setDescription("<:AuraDown:721175040376438824> | Insert the number less than **100.**")
        .setColor(colours.bot_white)
        if (args[0] > 100) return message.channel.send(numberLessThen100) 
        let numberMoreThen1 = new Discord.MessageEmbed()
        .setDescription("<:AuraDown:721175040376438824> | Insert the number more than **1.**")
        .setColor(colours.bot_white)
        if (args[0] < 2) return message.channel.send(numberMoreThen1)
        let error = new Discord.MessageEmbed()
        .setDescription(`<:AuraDown:721175040376438824> | Something went wrong while deleting the messages.`)
        .setColor(colours.bot_white)
        await message.delete()
        await message.channel.bulkDelete(args[0])
        let succesfull = new Discord.MessageEmbed()
        .setDescription(`<:AuraUpvote:721174999456809023> | I have deleted succesfully deleted **${message.size}/${args[0]}**`)
        .setColor(colours.bot_white)
