...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    .setDescription(" <:AuraError:729237692054896721>  | Please mention the user ```usage | a!kick [user] <reason>```")
    .setColor(colours.client_white)    
    if (!user) return message.channel.send(pleaseMention);
    let kickYourself = new Discord.MessageEmbed()
    .setDescription(" <:AuraError:729237692054896721>  | You can't kick yourself.")
    .setColor(colours.client_white)    
    if (user.id === message.author.id) return message.channel.send(kickYourself);
    let kickAura = new Discord.MessageEmbed()
    .setDescription(" <:AuraError:729237692054896721>  | You can't kick me.")
    .setColor(colours.client_white)    
    ...
    .setDescription(`<:AuraTick:729239339417993230>  | Successfully kicked **${user.tag}**`)
    .setColor(colours.client_white)
    .addFields(
    ...
    let unableToKick = new Discord.MessageEmbed()
        .setDescription(`<:AuraWarning:729241847284760686>   | I was unable to kick the member.`)
        .setColor(colours.client_white)    
